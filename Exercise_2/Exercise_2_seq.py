...
def grayscale_jpeg_conv_jit(image, grayscale_array, rows, cols, k_width, k_height, quantization_matrix): 
    ...
    dct_cos_rows = np.zeros((k_height, k_height), dtype=np.float32)
    idct_cos_rows = np.zeros((k_height, k_height), dtype=np.float32)
    ...
    dct_cos_cols = np.zeros((k_width, k_width), dtype=np.float32)
    idct_cos_cols = np.zeros((k_width, k_width), dtype=np.float32)
