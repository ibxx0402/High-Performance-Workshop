...
def bfsv_parallel(graph, source,processors):
    comm = MPI.COMM_WORLD
    ...
    if rank == 0:
        ...
        node_splits = np.full((split+split_offset,2),-1,dtype=np.int32)
        

        for i in range(0,split-split_offset):
    ...
    while True:
        ...
        lo_to_send = np.full(len(lo_nodes),-1)
        full_list = np.full(len(graph),-1)
...
if __name__ == "__main__":
    ...
    nasty_graph = np.array([[1,2,4],[3,4,-1],[4,5,-1],[6,-1,-1],[6,7,8],[7,-1,-1],[8,-1,-1],[8,-1,-1],[-1,-1,-1],[0,-1,-1],[-1,-1,-1],[-1,-1,-1]],dtype=np.int32)
    source = 0

    t = bfsv_parallel(graph,source,3)
